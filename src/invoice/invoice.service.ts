...
export class InvoiceService {
  ...
  async resendInvoiceEmail(orderId: string, role: UserRole) {
    ...
    if (isVendor) {
      ...
      html = generateVendorHTMLTemplate(invoiceData);
      ...
      html = generateCustomerHTMLTemplate(invoiceData);
  ...
  async generateInvoice(dto: InvoiceDto, type: UserRole, caller?: JwtPayload) {
    return this.prisma.$transaction(async (tx) => {
        ...
        if (!isAdmin) {
          ...
          if (!ownerUserId || ownerUserId !== caller.sub) {
            throw new ForbiddenException(
              'You can only download your own invoice',
            );
          }
        }
      }
      ...
      if (type === UserRole.VENDOR && !order.vendor_invoice_id) {
      ...
      if (type === UserRole.CUSTOMER && !order.customer_invoice_id) {
  ...
  private async getNextInvoiceSequence(
    ...
    try {
      ...
      const existing = await (tx as any).invoiceCounter.findUnique({
        where: { year },
      });
      if (!existing) {
  ...
  //     const htmlTemplate = generateVendorHTMLTemplate(invoiceData);
  ...
  private async createVendorInvoice(
    ...
    const { invoiceNumber, year, seq } = await this.generateInvoiceNumber(
      tx,
      InvoiceType.VENDOR,
    );
  ...
  //     const htmlTemplate = generateCustomerHTMLTemplate(invoiceData);
  //     const pdfBuffer = await generatePDF(htmlTemplate);
  ...
  async getInvoicesByOrderId(orderId: string, caller?: JwtPayload) {
    ...
    if (caller) {
      ...
      if (!isAdmin && !isVendor && !isCustomer) {
        throw new ForbiddenException(
          'You can only view invoices on your own orders',
        );
      }
    }
