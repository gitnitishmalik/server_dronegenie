...
export class InvoiceController{
    ...
    @ApiOperation({ summary: 'Download customer invoice PDF for an order (owner or admin only)' })
    ...
    async createCustomer(
        ...
        const generated = await this.invoiceService.generateInvoice(dto, UserRole.CUSTOMER, caller)
    ...
    @ApiOperation({ summary: 'Download vendor invoice PDF for an order (owner or admin only)' })
    @ApiResponse({ status: 200, description: 'Invoice Generated' })
    @ApiResponse({ status: 403, description: 'Not your invoice' })
    @ApiResponse({ status: 404, description: 'Order Not Found' })
    async createVendor(
        ...
        const generated = await this.invoiceService.generateInvoice(dto, UserRole.VENDOR, caller);
    ...
    @Get(":orderId")
    @ApiOperation({ summary: 'Get Invoice Records For An Order (customer, vendor, or admin)' })
    ...
    @ApiOperation({ summary: 'Admin: regenerate PDF from existing invoice row and re-send email' })
    ...
    adminResend(
        ...
        const mapped =
            upper === 'VENDOR' ? UserRole.VENDOR :
            upper === 'CUSTOMER' ? UserRole.CUSTOMER :
            null;
        if (!mapped) {
