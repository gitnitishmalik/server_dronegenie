...
const expandUnit = (u?: string | null) => {
  ...
  const map: Record<string, string> = {
    ...
    sqmt: 'square metres',
    sqmtr: 'square metres',
    sqmeter: 'square metres',
    sqmeters: 'square metres',
    sqmetre: 'square metres',
    sqmetres: 'square metres',
    'm²': 'square metres',
    m2: 'square metres',
    sqft: 'square feet',
    sqfeet: 'square feet',
  ...
  input_schema: {
    ...
    properties: {
      ...
      search: {
        type: 'string',
        description:
          'Optional case-insensitive keyword filter on service name.',
      },
    },
  },
};
...
const TOOL_LIST_OPEN_BIDS: Anthropic.Messages.Tool = {
  ...
  input_schema: {
    ...
    properties: {
      ...
      location_contains: {
        type: 'string',
        description: 'Optional substring filter on the bid location.',
      },
    },
  },
};
  ...
  input_schema: {
    ...
    properties: {
      bid_request_id: {
        type: 'string',
        description:
          "The customer's bid request id (from list_my_bid_requests).",
      },
    },
...
const ROLE_GUIDE_CUSTOMER = (
  name?: string,
) => `Audience: SIGNED-IN CUSTOMER${name ? ` (${name})` : ''}.
...
const ROLE_GUIDE_VENDOR = (
  name?: string,
) => `Audience: SIGNED-IN VENDOR${name ? ` (${name})` : ''}.
...
export class ChatService {
  ...
  constructor(
    ...
    this.dailyBudgetUsd = parseFloat(
      process.env.DAILY_CLAUDE_BUDGET_USD || '5',
    );
    if (!this.client) {
      this.logger.warn(
    ...
    try {
      ...
      if (name === 'list_open_bid_requests') {
        ...
        if (input?.location_contains) {
          where.location = {
            contains: String(input.location_contains),
            mode: 'insensitive',
          };
        }
      ...
      if (name === 'list_my_bid_requests') {
        if (!auth?.customerId)
          return JSON.stringify({ error: 'customer sign-in required' });
        const limit = Math.min(Math.max(Number(input?.limit) || 5, 1), 10);
        const rows = await this.prisma.bidRequest.findMany({
      ...
      if (name === 'list_replies_to_my_bid') {
        ...
        if (!owns)
          return JSON.stringify({
            error: 'bid request not found or not yours',
          });
        ...
        return JSON.stringify({
          ...
          replies: rows.map((r) => ({
            ...
            status: r.status,
            vendor_name:
              r.vendor?.comp_name || r.vendor?.representative || 'Vendor',
      ...
      if (name === 'list_my_bid_replies') {
        if (!auth?.vendorId)
          return JSON.stringify({ error: 'vendor sign-in required' });
        const limit = Math.min(Math.max(Number(input?.limit) || 5, 1), 10);
        const rows = await this.prisma.bidReply.findMany({
        ...
        return JSON.stringify({
          ...
          my_bid_replies: rows.map((r) => ({
            ...
            area:
              r.bidRequests?.area && r.bidRequests?.unit
      ...
      if (name === 'prepare_bid_action') {
        ...
        if (action === 'place' && !auth?.vendorId) {
          return JSON.stringify({
            error: 'Only vendors can place bids — please sign in as a vendor.',
          });
        }
        if (action === 'accept' && !auth?.customerId) {
          return JSON.stringify({
            error:
              'Only customers can accept bid replies — please sign in as a customer.',
          });
        }
        ...
        const url =
          action === 'place'
            ? reqId
              ? `${SITE_BASE}/vendor/bid-requests/${reqId}`
              : `${SITE_BASE}/vendor/bid-requests`
            : reqId
              ? `${SITE_BASE}/customer/bid-requests/${reqId}`
              : `${SITE_BASE}/customer/bid-requests`;
        const next_steps =
          action === 'place'
            ? [
      ...
      if (name === 'send_inquiry_email') {
        ...
        const fromName = String(input?.from_name || '')
          .trim()
          .slice(0, 100);
        const fromEmail = String(input?.from_email || '')
          .trim()
          .slice(0, 200);
  ...
  async reply(messages: Msg[], ip: string, auth?: ChatAuth) {
    ...
    const conversation: Anthropic.Messages.MessageParam[] = messages.map(
      (m) => ({
        role: m.role,
        content: m.content,
      }),
