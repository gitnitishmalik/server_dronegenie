...
import { CreateBidReplyDto, MilestoneDto, UpdateBidReplyDto } from "./dtos/bid-reply.dto";
...
function computeCustomerPrice(vendorPrice: number, dgCharges: number, type: DgChargeType): number {
...
function normaliseMilestones(raw: unknown): MilestoneDto[] {
  ...
  return raw.map((m: any, i: number) => {
    ...
    if (!Number.isFinite(va) || va <= 0) {
      throw new BadRequestException(`Milestone ${i + 1}: vendor_amount must be a positive number`);
    }
...
function assertMilestonesSumToPrice(milestones: MilestoneDto[], price: number) {
  const sumPaise = milestones.reduce((acc, m) => acc + Math.round((m.vendor_amount ?? 0) * 100), 0);
...
export class BidReplyService {
  ...
  async getReplyByBidRequestIdForCustomer(bidRequestId: string, caller: JwtPayload) {
    const bidRequest = await this.prisma.bidRequest.findUnique({
      where: { id: bidRequestId },
      include: {
    ...
    if (!isAdmin && bidRequest.customer?.user?.id !== caller.sub) {
      throw new ForbiddenException('You can only view bid replies on your own bid requests');
    }

    return bidRequest;
  }

  async update(id: string, dto: UpdateBidReplyDto, caller: JwtPayload, files?: Express.Multer.File[]) {
    ...
    if(bidReply.status !== ReplyStatus.PENDING) throw new BadRequestException("You can't edit a bid reply once accepted!")
    ...
    const priceChanged = dto.price !== undefined && dto.price !== bidReply.price;
    ...
    const normalisedMilestones = dto.milestones ? normaliseMilestones(dto.milestones) : undefined;
    if (normalisedMilestones) {
      assertMilestonesSumToPrice(normalisedMilestones, price);
    }

    const calculatedPrice = computeCustomerPrice(price, dgCharges, dgChargeType);
    ...
    const updatedReply = await this.prisma.bidReply.update({
      ...
      data: {
        ...
        ...(normalisedMilestones
          ? { proposed_milestones: normalisedMilestones as unknown as Prisma.InputJsonValue }
          : {}),
      },
    });
