    ...
    async create(dto: CreateFaqDto) {
        try {
            ...
            if (e instanceof Prisma.PrismaClientKnownRequestError && e.code === 'P2002') {
                ...
                if (Array.isArray(target) && target.length > 1) {
                    ...
                    if (target.includes('priorty') && target.includes('type')) {
                        ...
                        throw new BadRequestException(`Priority ${dto.priorty} is already used for type ${t}. Please choose a different priority for this type.`);
                    }
                }
            ...
            if (e?.code === 11000) {
                ...
                if (keys.length === 2 && keys.includes('priorty') && keys.includes('type')) {
                    const t = keyValue.type ?? dto.type ?? 'this type';
                    const p = keyValue.priorty ?? dto.priorty;
                    throw new BadRequestException(`Priority ${p} is already used for type ${t}.`);
                }
                if (keys.length === 1) {
                    const field = keys[0];
            ...
            if (e instanceof Prisma.PrismaClientKnownRequestError && e.code === 'P2002') {
                ...
                if (Array.isArray(target) && target.length > 1) {
                    if (target.includes('priorty') && target.includes('type')) {
                        ...
                        throw new BadRequestException(`Priority ${p} is already used for type ${t}. Please choose a different priority for this type.`);
                    }
                }
                throw new BadRequestException(getFriendlyUniqueMessage(target));
            }
