...
export class VendorPayoutController {
  ...
  @ApiOperation({
    summary: 'Create Razorpay Route linked account for this vendor (gated)',
  })
  create(
  ...
  @Roles(UserRole.VENDOR)
  @ApiOperation({
    summary: "Get the logged-in vendor's Route account state (gated)",
  })
  ...
  @ApiOperation({
    summary: 'Refresh Route account status from Razorpay (gated)',
  })
  ...
  @ApiOperation({
    summary: 'Admin: list all vendor payouts with filters (gated)',
  })
  ...
  @Roles(UserRole.ADMIN)
  @ApiOperation({
    summary: 'Admin: stream CSV export of vendor payouts (gated)',
  })
  async exportCsv(@Query() dto: AdminPayoutListDto, @Res() res: Response) {
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename="vendor-payouts-${stamp}.csv"`,
    );
    try {
